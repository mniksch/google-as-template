...
def create_project(cfg):
    ...
    try:
        # Create project
        request = {"title": cfg["script_name"], "parentId": cfg["project_dir"]}
        ...
        cfg["logger"].info(
            "https://console.cloud.google.com/home/dashboard?project=" + creds.project
        )
...
def push_scripts(cfg, service=None, scriptId=None):
    ...
    files = [
        ...
        for filename, code_body in filework.grab_js_files(
            cfg["local_script_dir"]
        ).items()
    ]
    files.append(
        {"name": "appsscript", "type": "JSON", "source": filework.build_manifest(cfg)}
    )
    request = {"files": files}
    cfg["logger"].info(
        "Pushing scripts",
    ...
    response = (
        service.projects().updateContent(body=request, scriptId=scriptId).execute()
    )
    return response
...
def pull_scripts(cfg):
    ...
    response = (
        ...
        .projects()
        .getContent(scriptId=googleapi.ScriptSettings(cfg).get_script_id())
        .execute()
    )
...
if __name__ == "__main__":
    if len(sys.argv) != 2:
        print("Usage: {} [{}]".format(sys.argv[0], "/".join(targets.keys())))
    else:
